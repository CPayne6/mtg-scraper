...
const proxyUrl = 'https://api.proxyscrape.com/v4/free-proxy-list/get?request=displayproxies&timeout=10000&' + params.toString()

const proxies: { ip: string; disabled?: boolean }[] = []

let proxiesPromise: Promise<void> = new Promise(() => {})
...
export const getProxyUrl = async (index?: number) => {
  ...
  return proxies[index === undefined ? Math.floor(Math.random() * proxies.length) : index]
}
