...
const input = {
  product: {
    title: '  Lightning Bolt [M11] (Foil)  ', vendor: 'Magic 2011', productType: 'MTG Single',
    descriptionHtml: '<p>collector: 123</p>', tags: ['Magic', 'Set:M11', 'English'], handle: 'lightning-bolt',
    onlineStoreUrl: 'https://example.test/products/lightning-bolt', availableForSale: true,
    images: [{ url: 'https://example.test/bolt.jpg' }],
  },
  variant: {
    id: 'gid://shopify/ProductVariant/1', title: 'Near Mint', sku: 'M11-123-EN-FO', availableForSale: true,
...
const profile = (source: string, transform: object) => ({
  kind: 'mapping' as const, version: 1 as const,
...
describe('Storefront parser profile grammar', () => {
  it.each([
    ...
    ['split', 'variant.sku', { type: 'split', delimiter: '-', index: 1 }, '123'],
    ['bracketGroup', 'product.title', { type: 'bracketGroup', index: 0 }, 'M11'],
    ['parenthesisGroup', 'product.title', { type: 'parenthesisGroup', index: 0 }, 'Foil'],
    ['regexCapture', 'variant.sku', { type: 'regexCapture', pattern: '^([A-Z0-9]+)-', group: 1 }, 'M11'],
    ['regexReplace', 'variant.sku', { type: 'regexReplace', pattern: '-EN-', replacement: '-' }, 'M11-123-FO'],
    ['stripTokens', 'product.title', { type: 'stripTokens', values: ['[M11]', '(Foil)'] }, 'Lightning Bolt'],
    ['optionValue', 'variant.selectedOptions', { type: 'optionValue', name: 'condition' }, 'Near Mint'],
    ['tagValue exact', 'product.tags', { type: 'tagValue', mode: 'exact', value: 'magic' }, 'Magic'],
    ['tagValue prefix', 'product.tags', { type: 'tagValue', mode: 'prefix', value: 'set:' }, 'Set:M11'],
  ...
  it('normalizes booleanTokens with false taking precedence', () => {
    const mapping: any = { kind: 'mapping', version: 1, fields: { cardName: { candidates: [{ value: 'Bolt' }] }, foil: { candidates: [{ source: 'variant.selectedOptions', transforms: [
      { type: 'optionValue', name: 'Finish' }, { type: 'booleanTokens', true: ['foil'], false: ['foil'] },
    ] }] } } };
    expect(evaluateStorefrontParserProfile(mapping, input as any).foil).toBe(false);
  });

  it('honours predicates, candidate fallback, and boolean false', () => {
    const tested = {
      kind: 'mapping' as const, version: 1 as const, fields: {
        cardName: { candidates: [
          { value: 'wrong', when: [{ source: 'product.vendor', operator: 'equals', value: 'Nope' }] },
    ...
    expect(evaluateStorefrontParserProfile(tested, input as any)).toEqual({
      cardName: 'Lightning Bolt [M11] (Foil)', foil: false,
    });
  });

  it.each(['equals', 'notEquals', 'contains', 'notContains', 'regex', 'isEmpty', 'notEmpty'] as const)(
    'supports %s predicates',
    (operator) => {
      const value = operator === 'equals' ? 'Magic 2011' : operator === 'notContains' ? 'sealed' : operator === 'isEmpty' ? '' : operator === 'regex' ? undefined : 'magic';
      const predicate: any = { source: operator === 'isEmpty' ? 'variant.sku' : 'product.vendor', operator, value, pattern: 'magic', flags: 'i' };
      if (operator === 'isEmpty') predicate.source = 'product.onlineStoreUrl';
      const result = evaluateStorefrontParserProfile({ kind: 'mapping', version: 1, fields: {
        cardName: { candidates: [{ value: 'matched', when: [predicate] }, { value: 'fallback' }] },
      } }, input as any);
      expect(result.cardName).toBe(operator === 'isEmpty' ? 'fallback' : 'matched');
    },
  );
  ...
  it('normalizes connection edges and nodes into identical profile inputs', () => {
    const product = {
      ...input.product,
      images: { nodes: input.product.images }, variants: { nodes: [input.variant] },
    };
    const edges = { ...product, images: { edges: input.product.images.map(node => ({ node })) }, variants: { edges: [{ node: input.variant }] } };
    expect(normalizeStorefrontProfileInputs(edges as any)).toEqual(normalizeStorefrontProfileInputs(product as any));
  });

  it('validates malformed profiles and profile sample requirements', () => {
    expect(validateStorefrontParserProfile({ kind: 'mapping', version: 2, fields: {} }).valid).toBe(false);
  ...
  it('applies Art Series exclusions with product and all-variant scope', () => {
    const mapping: any = { kind: 'mapping', version: 1, fields: { cardName: { candidates: [{ source: 'product.title' }] } }, exclusions: [
      { reason: 'art-series', scope: 'product', predicate: { source: 'product.tags', operator: 'contains', value: 'magic' } },
    ] };
    expect(ProfiledStorefrontCardParser.isArtSeries(mapping, [input as any])).toBe(true);
    mapping.exclusions[0] = { reason: 'art-series', scope: 'allVariants', predicate: { source: 'variant.title', operator: 'equals', value: 'Near Mint' } };
    expect(ProfiledStorefrontCardParser.isArtSeries(mapping, [input as any])).toBe(true);
  });
});
