...
export function ColorModeIcon() {
  const { colorMode } = useColorMode();
  return colorMode === 'dark' ? <DarkMode /> : <LightMode />;
}
...
export const ColorModeButton = React.forwardRef<HTMLButtonElement, ColorModeButtonProps>(
  function ColorModeButton(props, ref) {
    ...
    if (!mounted) return <Skeleton variant="circular" width={36} height={36} />;
    return (
      <IconButton onClick={toggleColorMode} aria-label="Toggle color mode" size="small" ref={ref} {...props}>
        <ColorModeIcon />
      </IconButton>
    );
  },
