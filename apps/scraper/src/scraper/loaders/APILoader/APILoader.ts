...
export class APILoader extends HTTPLoader {
  ...
  formatPath(path: APILoaderConfig['api']['path'], page: string, term: string) {
    let finalPath =
      path instanceof RegExp ? (this.runRegex(path, page) ?? '') : '';
  ...
  formatParams(
    params: APILoaderConfig['api']['params'],
    page: string,
    term: string,
  ) {
    const finalParams = new URLSearchParams(
      params instanceof RegExp ? this.runRegex(params, page) : undefined,
    );
    ...
    if (Array.isArray(params)) {
      for (const param of params) {
        if (Array.isArray(param)) {
          ...
          const parsedValue =
            value instanceof RegExp ? this.runRegex(value, page) : value;
          if (key && parsedValue) {
            finalParams.set(
              key,
  ...
  formatBody(body: APILoaderConfig['api']['body'], page: string, term: string) {
    const finalBody =
      body instanceof RegExp
        ? JSON.parse(this.runRegex(body, page) ?? '{}')
        : {};
  ...
  async search(name: string, params?: URLSearchParams) {
    ...
    if (
      ...
      (this.apiConfig.cacheTimeout ?? defaultCacheTimeout)
    ) {
      const path =
    ...
    const apiParams = this.formatParams(
      this.apiConfig.api.params,
      this.cachedPage,
      name,
    );
    const apiPath = this.formatPath(
      this.apiConfig.api.path,
      this.cachedPage,
      name,
    );
    const baseUrl =
      this.apiConfig.api.baseUrl instanceof RegExp
        ? this.runRegex(this.apiConfig.api.baseUrl, this.cachedPage)
        : this.apiConfig.api.baseUrl;
    const body = this.formatBody(
      this.apiConfig.api.body,
      this.cachedPage,
      name,
    );
