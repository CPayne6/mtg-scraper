...
const defaultConfig: _401ParserSearchConfig = {
  ...
  params: new URLSearchParams(
    'sort_by=relevency&with_product_attributes=true&narrow=[["In Stock","True"]]&products_per_page=40',
  ),
...
export class _401Parser implements Parser {
  ...
  async extractItems(data: string) {
    ...
    // If items is missing or not an array, card not found (not an error)
    ...
    for (const item of parsedData.items) {
      ...
      for (const innerItem of item.vra) {
        ...
        if (productCorrect && attrMap['Sellable'] !== false) {
          const cardName =
            item.l.match(cardNameRegex)?.[1].split(' - ')[0] ?? item.l;
          const splitSku = (
