...
- [ ] Make merchant inventory quantity a platform-neutral offer capability.
  - Audit the existing `card_variants.quantity` persistence path and expose nullable quantity in listing/offer API DTOs.
  - Display an exact count such as “3 in stock” when a platform supplies it; retain generic in-stock text when quantity is unavailable.
  - Conduct Commerce quantity is per condition/finish variant and should drive `inStock = quantity > 0`.
  - Keep Shopify quantity nullable: Storefront access does not provide it without merchant-authorized Admin inventory access.
  - Add zero/one/many/missing-quantity fixtures and UI/API coverage.
